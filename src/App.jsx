...
const LOCAL_CLOSING_KEY = "inventory_local_closing_rows_v3";
const RECENT_INVENTORY_KEY = "inventory_recent_rows_v1";
...
function rowKey(row) {
  return `${row.outletId || ""}__${row.barId || ""}__${row.barcode || row.productCode || row.productId || row.id || ""}`;
}

function saveLocalClosingRow(row) {
  const savedRow = { ...row, id: row.id || rowKey(row), localSavedAt: new Date().toISOString() };
  const key = rowKey(savedRow);
  const existing = getLocalClosingRows().filter((r) => rowKey(r) !== key);
  localStorage.setItem(LOCAL_CLOSING_KEY, JSON.stringify([savedRow, ...existing]));
  saveRecentInventoryRow(savedRow);
}

function getRecentInventoryRows() {
  try {
    return JSON.parse(localStorage.getItem(RECENT_INVENTORY_KEY) || "[]");
  } catch {
    return [];
  }
}

function saveRecentInventoryRow(row) {
  const savedRow = { ...row, recentSavedAt: new Date().toISOString() };
  const existing = getRecentInventoryRows();
  localStorage.setItem(RECENT_INVENTORY_KEY, JSON.stringify([savedRow, ...existing].slice(0, 12)));
}
...
function mergeLocalClosingRows(serverRows) {
  ...
  getLocalClosingRows().forEach((localRow) => {
    ...
    const index = merged.findIndex((r) => rowKey(r) === key);
    if (index >= 0) merged[index] = { ...merged[index], ...localRow, id: merged[index].id || localRow.id || key };
    else merged.unshift({ ...localRow, id: localRow.id || key });
  });
  return merged;
}
...
function MachineStatusCard({ onBarcodeDetected, onWeightDetected, currentWeight, onResetWeight, registerReset }) {
  ...
  function handleIncomingWeight(raw) {
    ...
    const samples = [...weightSamplesRef.current, cleanWeight].slice(-2);
    weightSamplesRef.current = samples;

    if (samples.length < 2) {
      setWeightStatus("Reading 1/2");
      return;
    }

    const quickWeight = Math.round(samples.reduce((a, b) => a + b, 0) / samples.length);
    weightLockedRef.current = true;
    setWeightStatus("Reading Done");
    onWeightDetected(quickWeight);
  }
  ...
  useEffect(() => {
    registerReset?.(resetStableWeight);
  }, []);
  ...
  return (
    <StatCard title="Device Status" value={scaleStatus} sub={`scanner: ${scannerStatus}`} icon="📡">
      ...
      <p className="small" style={{ marginTop: 8 }}>Ready for barcode scan and quick 2-value weight reading.</p>
      <input
        ref={scannerInputRef}
        value={barcodeValue}
...
function InventoryReading({ disabled, row, products, brandSearch, setBrandSearch, onPickProduct, closingFullBottle, setClosingFullBottle, closingEmptyBottle, setClosingEmptyBottle, closingOpenBottleMl, setClosingOpenBottleMl, onSave, onReadAgain }) {
  const active = !!row;
  return (
    <div className={`reading ${disabled ? "disabled" : ""}`}>
      <div className="readingTop">
        ...
        <div className="readingRight">
          <div className="currentReading">
            <p className="muted">Remaining ML</p>
            <h2>{closingOpenBottleMl !== "" ? `${closingOpenBottleMl} ML` : active ? `${row.closingOpenBottleRemainingMl || 0} ML` : "--"}</h2>
            <Button variant="secondary" className="readAgainInline" onClick={onReadAgain}>Read Again</Button>
          </div>
        </div>
      </div>
      ...
      <div className="rowButtons">
        ...
        <Button variant="secondary" onClick={onReadAgain}>Read Next Bottle</Button>
...
function RecentInventory({ rows }) {
  return (
    <div className="recentInventory">
      <div className="cardHead compactHead">
        <div>
          <h2>Recent Inventory</h2>
          <p className="muted">Latest bottles saved from this system.</p>
        </div>
      </div>
      <div className="recentList">
        {rows.length === 0 && <div className="historyItem"><b>No recent inventory yet.</b><span>Save closing inventory to see it here.</span></div>}
        {rows.slice(0, 6).map((r, index) => (
          <div className="recentItem" key={`${rowKey(r)}_${r.recentSavedAt || r.localSavedAt || index}`}>
            <div>
              <b>{r.name || r.brandName || r.barcode}</b>
              <span>{r.outletName || r.outlet || "Outlet"} / {r.barName || r.location || "Location"}</span>
            </div>
            <div className="recentNumbers">
              <strong>{Number(r.closingOpenBottleRemainingMl ?? r.remainingML ?? r.totalOpenBottle ?? 0)} ML</strong>
              <small>Full: {Number(r.closingFullBottleCount ?? r.closingFullBottle ?? r.totalFullBottle ?? 0)}</small>
            </div>
          </div>
        ))}
      </div>
...
function Dashboard({ onLogout }) {
  ...
  const [recentInventory, setRecentInventory] = useState(() => getRecentInventoryRows());
  const resetWeightReadRef = useRef(null);
  ...
  const selectedProduct = products.find((p) =>
    ...
    p.barcode === lastBarcode ||
    p.productCode === selectedProductId ||
    p.name === brandSearch
  );
  const latestRow = activeRows.find((r) =>
    r.productId === selectedProductId ||
    r.productId === selectedProduct?.id ||
    r.barcode === selectedProduct?.barcode ||
    r.productCode === selectedProduct?.productCode ||
    r.name === selectedProduct?.name
  ) || activeRows[0];

  function calculateRemainingMl(weight, product) {
    const numericWeight = Math.round(Number(weight));
    if (!Number.isFinite(numericWeight)) return "";
    if (!product) return String(Math.max(0, numericWeight));
    const bottleSize = Number(product.bottleSizeMl || product.bottleSize || 750);
    const emptyBottleWeight = Number(product.emptyBottleWeightG);
    const emptyWeightSafe = Number.isFinite(emptyBottleWeight) ? emptyBottleWeight : 0;
    const remaining = Math.round(numericWeight - emptyWeightSafe);
    const upperLimit = Number.isFinite(bottleSize) && bottleSize > 0 ? bottleSize : 750;
    return String(Math.max(0, Math.min(upperLimit, remaining)));
  }

  function updateRemainingFromWeight(weight, product = selectedProduct) {
    const remaining = calculateRemainingMl(weight, product);
    if (remaining !== "") setClosingOpenBottleMl(remaining);
  }
  ...
  async function handleBarcodeDetected(code) {
    ...
    try {
      ...
      setProducts((prev) => prev.some((p) => p.barcode === product.barcode || p.id === product.id) ? prev : [product, ...prev]);
      setSelectedProductId(product.id);
      setBrandSearch(product.name);
      if (currentWeight !== "") updateRemainingFromWeight(currentWeight, product);
      setStatus(`Barcode scanned: ${code}. ${product.name} found.`);
    } catch {
      setStatus(`Barcode scanned: ${code}. Product not found in database. Remaining ML will show as raw weight until product is selected.`);
      setBrandSearch(code);
      if (currentWeight !== "") updateRemainingFromWeight(currentWeight, null);
    }
  }

  function handleWeightDetected(weight) {
    setCurrentWeight(weight);
    updateRemainingFromWeight(weight);
  }

  useEffect(() => {
    if (currentWeight !== "") updateRemainingFromWeight(currentWeight);
  }, [currentWeight, selectedProductId, lastBarcode, products.length]);

  function onPickProduct(product) {
    setSelectedProductId(product.id);
    setBrandSearch(product.name);
    if (currentWeight !== "") updateRemainingFromWeight(currentWeight, product);
    else setClosingOpenBottleMl("");
  }
  ...
  async function saveManualEntry() {
    ...
    try {
      ...
      setRecentInventory(getRecentInventoryRows());
          ...
          <>
            <section className="stats">
              ...
              <MachineStatusCard
                ...
                registerReset={(fn) => { resetWeightReadRef.current = fn; }}
              />
            </section>
            ...
            <Card>
              ...
              <InventoryReading
                ...
                onReadAgain={() => {
                  resetWeightReadRef.current?.();
                  setClosingOpenBottleMl("");
                  setCurrentWeight("");
                }}
              />
              <RecentInventory rows={recentInventory} />
            </Card>
          </>
        )}
